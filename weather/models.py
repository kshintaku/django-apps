...
class LocationManager(models.Manager):
  def create_location(self, city, lat, lon, tz, tzo):
    loc = self.create(city=city, latitude=lat, longitude=lon, timezone=tz, timezone_offset=tzo)
    return loc
...
class WeatherManager(models.Manager):
  def create_weather(self, time, temp, uv, ws):
    weather = self.create(weather_time=time, current_temp=temp, current_uv=uv, current_wind_speed=ws)
    return weather
